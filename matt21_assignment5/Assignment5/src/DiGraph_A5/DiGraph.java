...
public class DiGraph implements DiGraph_Interface {
	...
	public boolean addNode(long idNum, String label) {
		if ((idNum < 0L) || 
				(this.vertexIDs.contains(Long.valueOf(idNum))) || 
				(label == null) || 
	...
	public boolean addEdge(long idNum, String sLabel, String dLabel, long weight, String label) {
		...
		if ((!this.vertices.containsKey(sLabel)) || 
				(!this.vertices.containsKey(dLabel)) || 
	...
	public String[] topoSort() {
		int size = this.vertexIDs.size();  						//Variable for size/ total number of vertices
		
		String[] toposort = new String[size];					//The string array we need to return

		Queue<Vertex> nodes = new LinkedList<Vertex>(); 		//My Queue that needs to be initialized as a LinkedList 
																//because a Queue is an ADT that implements a LinkedList in the Java Util library
		int count = -1;
		
		
		for (Vertex a : this.vertices.values()) {				//I use this for loop to assign the indegree to each vertex
			a.initEd();											//	
		...
		while (!nodes.isEmpty()) {
			...
			Vertex temp = nodes.poll();							//Everytime the while-loop goes around, i poll off the 
																//top of the queue, and save the label in the count location in the string[ ]

			toposort[count] = temp.Label;
			Edge[] tempAr = temp.OutEdge.values().toArray(new Edge[temp.OutEdge.size()]);
																	//I collect all the edges that my vertex points to and then convert it into an array

			
			for (int i = 0; i < tempAr.length; i++) {
				Vertex des = tempAr[i].dest;
				des.subEd();;
				if ((des.getEd() == 0)) {						//I go to all of those vertices, and decrement their indegree by
																//1, if any of them hit 0, i throw them into the queue, then I rinse and repeat
